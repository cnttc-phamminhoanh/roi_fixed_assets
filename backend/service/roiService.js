...
class ROIService {
  ...
  async getROIData() {
    try {
      ...
      // console.log(
      //   `📊 Trạng thái database: ${database.isConnected ? "✅ Connected" : "❌ Not connected"}`,
      // );
      ...
      const query = `
        select
          d.dept_name, concat(concat(a.sheet_no, concat('/', a.sheet_id)), concat(' - ', a.fa_desc)) as fa_desc, 
      ...
      try {
        ...
        // console.log(
        //   `✅ Query thành công, nhận được ${results ? results.length : 0} dòng dữ liệu`,
        // );
  ...
  transformData(results) {
    ...
    const transformed = results.map((item, index) => {
      ...
      const budgetROI =
        parseFloat(item.budget_roi) || parseFloat(item.roi_plan) || 0;
      const actualROI =
        parseFloat(item.actual_roi) || parseFloat(item.roi_act) || 0;
      ...
      return {
        ...
        estimatedPaybackTime:
          parseFloat(item.estimated_payback_time) || item.yr_payback || 0,
        estimatedPaybackDate: item.estimated_payback_date || item.payback_date,
        budgetQuantity:
          parseInt(item.budget_quantity) || parseInt(item.plan_qty) || 0,
        budgetAmount:
          parseFloat(item.budget_amount) || parseFloat(item.plan_amt) || 0,
        budgetBenefit:
          parseFloat(item.budget_benefit) || parseFloat(item.plan_benifit) || 0,
        budgetROI: budgetROI,
        actualQuantity:
          parseInt(item.actual_quantity) || parseInt(item.in_qty) || 0,
        actualAmount:
          parseFloat(item.actual_amount) || parseFloat(item.act_amt) || 0,
        actualBenefit:
          parseFloat(item.actual_benefit) || parseFloat(item.act_benifit) || 0,
        actualROI: actualROI,
  ...
  async updateActualBenefit(planNo, planId, benefitValue) {
    try {
      // console.log(
      //   `📝 updateActualBenefit - planNo: ${planNo}, planId: ${planId}, benefit: ${benefitValue}`,
      // );
      ...
      // console.log(
      //   `📝 Params: benefitValue=${benefitValue}, planNo=${planNo}, planId=${planId}`,
      // );
      ...
      if (affectedRows === 0) {
        // console.warn(
        //   `⚠️ Không tìm thấy bản ghi với plan_no=${planNo} và plan_id=${planId}`,
        // );
        return {
          success: true,
  ...
  async updateMultipleBenefits(updates) {
    try {
      // console.log(
      //   `📝 updateMultipleBenefits - Số lượng cập nhật: ${updates.length}`,
      // );
      ...
      if (!database.isConnected) {
        ...
        const updatedData = sampleData.map((item) => {
          const update = updates.find(
            (u) => u.planNo === item.planNo && u.planId === item.planId,
          );
          if (update) {
            return {
              ...item,
              ...
              actualROI:
                item.actualAmount > 0
                  ? (update.benefitValue / item.actualAmount) * 100
                  : 0,
            };
          }
          return item;
